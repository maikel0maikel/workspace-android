...
public class MyAlbumDAO extends _MyDAOAbstract<MyAlbumDAO, MyAlbum> implements
		_MyDAOInterface<MyAlbumDAO, MyAlbum> {
	/**
	...
	public Integer insert(MyAlbum obj) {
		...
		try {
			MyAlbum tmp = getDao().queryBuilder().where()
					.eq(MyAlbum.NAME_F, obj.getName()).queryForFirst();
			if (tmp == null) {
				super.insert(obj);
			} else {
				obj.setId(tmp.getId());
	...
	public ArrayList<MySong> getSongs(MyAlbum obj) {
		...
		if (getSource() == MySource.DB_SOURCE) {
			try {
				 ...
				 */
				List<MySong> tmp = getGlobalDAO().getMySongDAO().getDao()
						.queryForEq(MySong.ALBUM_ID, obj.getId());
				for (MySong item : tmp) {
