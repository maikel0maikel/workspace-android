...
public class MyPlayListDAO extends _MyDAOAbstract<MyPlayListDAO, MyPlayList>
	...
	public Integer insert(MyPlayList obj) {
		...
		try {
			MyPlayList tmp = getDao().queryBuilder().where()
					.eq(MyPlayList.NAME_F, obj.getName()).queryForFirst();
			if (tmp == null) {
				super.insert(obj);
			} else {
				obj.setId(tmp.getId());
	...
	public ArrayList<MySong> getSongs(MyPlayList obj) {
		...
		if (getSource() == MySource.DB_SOURCE) {
			try {
				List<MySong_MyPlayList> tmp = getGlobalDAO()
						.getMySong_MyPlayListDAO().getDao()
						.queryForEq(MySong_MyPlayList.PLAYLIST_ID, obj.getId());
				for (MySong_MyPlayList item : tmp) {
					song = item.getSong();
					song.setDao(getGlobalDAO().getMySongDAO());
					song.setLoaded(false);// not true because MySong_MyPlayList
											// only load ID
					re.add(song);
				}
				return re;
