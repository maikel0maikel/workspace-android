...
/**
 * Lam viec truc tiep voi doi tuong MySong trong CSDL de truy van ra danh sach
 * cac doi tuong
 * 
 * @author quocdunginfo
 * 
 */
public class MySongDAO extends _MyDAOAbstract<MySongDAO, MySong> implements
		_MyDAOInterface<MySongDAO, MySong> {
	...
	/**
	 * Get all song in DB Only support DB SOURCE at this time
	 */
	@Override
	...
	public Boolean update(MySong obj) {
		...
		/*
		 * //backup source state Integer bk_source =
		 * obj.getGlobalDAO().getSource(); //first change to DB_SOURCE first to
		 * ensure mp3 file not deleted
		 * obj.getGlobalDAO().setSource(MySource.DB_SOURCE); //get tmp info for
		 * preservation String absPath = obj.getPath().getAbsPath(); String
		 * album = obj.getAlbum().getName(); String artist =
		 * obj.getArtist().getName(); String title = obj.getTitle(); //update
		 * tag info to mp3 file on disk File src = new
		 * File(obj.getPath().getAbsPath()); MusicMetadataSet src_set; try {
		 * //read metaset for holder src_set = new MyID3().read(src); if
		 * (src_set == null) // perhaps no metadata { return false; } else {
		 * //create metavalues MusicMetadata metadata = new MusicMetadata("");
		 * metadata.setArtist(artist); metadata.setAlbum(album);
		 * metadata.setSongTitle(title); new MyID3().update(src, src_set,
		 * metadata); } } catch (Exception e) { // TODO Auto-generated catch
		 * block e.printStackTrace(); return false; }
		 * 
		 * //delete current in DB, do not remove from disk obj.delete();
		 * 
		 * //clone new one for clean insert // switch to DISK SOURCE and then
		 * load mp3 file agian => re-insert to DB MySong new_obj = new MySong();
		 * new_obj.setDao(this);
		 * new_obj.getGlobalDAO().setSource(MySource.DISK_SOURCE);
		 * new_obj.setPath(absPath); new_obj.insert(); //copy source from
		 * obj=>new_obj new_obj.getGlobalDAO().setSource(bk_source); obj =
		 * new_obj; return true;
		 */
	}
		...
		if (getSource() == MySource.DISK_SOURCE) {
			...
			try {
				String[] tmp = getDao().queryBuilder()
						.selectColumns(MySong.ALBUM_ID).where()
						.eq(MySong.ID_F, obj.getId()).queryRawFirst();
	...
	/**
	 * Ho tro set Loaded Khong ho tro pass DAO, phan quan ly pass DAO thuoc
	 * MySong
	 */
	@Override
	public void load(MySong obj) {
		...
		if (getSource() == MySource.DB_SOURCE) {
			...
			if (ID3_NATIVE_ANDROID) {
				...
				obj.setDuration(MyNumberHelper.stringToLong(tmp));
				...
				MyFormat format = new MyFormat(obj.getPath().getFileExtension(
						false));
			...
			// use external myID3 library
				...
				try {
					...
					if (src_set == null) // perhaps no metadata
					{
						...
						obj.setDuration(MyNumberHelper.stringToLong(metadata
								.getDurationSeconds()));
					}
				} catch (IOException e) {
					// TODO Auto-generated catch block
	...
	public Boolean delete(MySong obj, Boolean removeFromDisk) {
		...
		if (removeFromDisk) {
			...
			if (getSource() == MySource.DISK_SOURCE && obj.getPath() != null
			...
			obj.getPath().getAbsPath();// trigger load absPath before delete
										// from DB
			// call delete
			obj.delete();
